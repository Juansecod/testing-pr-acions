...
describe('SolicitudController', () => {
  ...
  const mockSolicitudService = {
    ...
    getSolicitudById: jest.fn((id) => (id === 1 ? { id, estado: 'pendiente' } : null)),
  ...
  describe('aprobarSolicitud', () => {
    it('debería aprobar una solicitud', async () => {
      const result = await controller.aprobarSolicitud(1, { salaId: 101, horaInicio: '08:00', horaFin: '10:00' });
      expect(result).toEqual({
        status: HttpStatus.OK,
        message: 'La solicitud con ID: 1 ha sido aprobada.',
      });
      expect(service.aprobarSolicitud).toHaveBeenCalledWith(1, 101, '08:00', '10:00');
    });
  });
