...
export class SolicitudController {
  ...
  @ApiResponse({ status: 200, description: 'Return all data about solicitudes' })
  ...
  @ApiResponse({ status: 200, description: 'Return all data about solicitudes where are approved' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @Get('/aprobadas')
  ...
  @ApiResponse({ status: 200, description: 'Return all data about solicitudes where are declined' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @Get('/rechazadas')
  ...
  @ApiResponse({ status: 200, description: 'Return all data about solicitudes where are pending to approve or decline' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({ status: 200, description: 'Return all data about solicitud with id parameter' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
...
  }
  ...
  async aprobarSolicitud(
    ...
    const solicitudAprobada = await this.solicitudService.aprobarSolicitud(
      id,
      body.salaId,
      body.horaInicio,
      body.horaFin,
  ...
  async rechazarSolicitud(
    ...
    const solicitudAprobada = await this.solicitudService.rechazarSolicitud(id);
    return {
