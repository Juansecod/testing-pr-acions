...
  }
  ...
  async obtenerIntervalosIndividuales(idSolicitud: number): Promise<{ id_sala: number; intervalos: { inicio: string; fin: string }[] }[]> {
    const solicitud = await this.solicitudRepository.findOne({
      where: { id_solicitudes: idSolicitud, Estado: 2 },
    });
  ...
  async obtenerIntervalosCombinados(idSolicitud: number): Promise<{ id_sala: number; intervalos: { inicio: string; fin: string }[] }[]> {
    const solicitud = await this.solicitudRepository.findOne({
      where: { id_solicitudes: idSolicitud, Estado: 2 },
    });
    ...
    const disponibilidadSalas = await Promise.all(salasCombinadas.map(async ({ id, individuales }) => {
      ...
      const solicitudesIndividuales = await Promise.all(individuales.map(async (salaId) => {
        return this.solicitudRepository.find({
          where: {
            Estado: 1,
      ...
      const todasSolicitudes = [...solicitudesCombinadas, ...solicitudesIndividuales.flat()];
      
      todasSolicitudes.sort((a, b) => a.Hora_inicio.localeCompare(b.Hora_inicio));
      ...
      todasSolicitudes.forEach((reserva) => {
        ...
        horaDisponibleInicio = reserva.Hora_final > horaDisponibleInicio ? reserva.Hora_final : horaDisponibleInicio;
      });
